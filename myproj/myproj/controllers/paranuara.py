...
FRUITS = set(["apple", "banana", "cucumber", "orange", "strawberry"])

VEGETABLES = set(["beetroot", "carrot", "celery"])
    ...
    def get_all(self, **kw):
        ...
        for entry, food in [("vegetables", VEGETABLES), ("fruits", FRUITS)]:
            if validated_entries.get(entry) == True:
                res.update({entry: list(food.intersection(set(person.favouriteFood)))})

        if (
            validated_entries.get("vegetables") is None
            and validated_entries.get("fruits") is None
        ):
...
class CommonFriendsAPIController(EasyCrudRestController):
    ...
    def get_one(self, friend_index, **kw):
        ...
        persons = M.People.query.find(
            {"index": {"$in": [int(index), friend_index]}},
            {
                "name": 1,
                "age": 1,
                "address": 1,
        ...
        common = set([p0["index"] for p0 in persons[0].friends]).intersection(
        ...
        log.debug(
            "common friends of {} and {},  {}".format(
                persons[0].index, persons[1].index, common
            )
        )
        ...
        for p in common_friends:
            ...
            cfriends.append(d)
        return dict(
            model="CommonFriends", value=dict(people=people, common_friends=cfriends)
        )
...
class PeopleAPIController(EasyCrudRestController):
    ...
    @validate({"index": Int(not_empty=True)})
    @expose("json", inherit=True)
...
class EmployeesAPIController(EasyCrudRestController):
    ...
    def get_all(self):
        ...
        errors = []
        try:
            company_id = Int().to_python(
                request.controller_state.routing_args.get("company_id")
            )
        ...
        if not errors:
            return {
                "model": "Employees",
                "value": dict(company=company, employees=employees),
            }
        else:
...
class CompanyAPIController(EasyCrudRestController):
    ...
    @validate({"company_id": Int(not_empty=True)})
    @expose("json", inherit=True)
