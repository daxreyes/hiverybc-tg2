...
As myproj grows and the authentication method changes, only these tests
should be updated.
...
class TestAuthentication(TestController):
    ...
    def test_forced_login(self):
        ...
        ok_(
            "authtkt" in initial_page.request.cookies,
            "Session cookie wasn't defined: %s" % initial_page.request.cookies,
        )
        ok_(
            initial_page.location.startswith("http://localhost/admin/"),
    ...
    def test_logout(self):
        ...
        resp = self.app.get(
            "/login_handler?login=manager&password=managepass", status=302
        )
    ...
    def test_failed_login_keeps_username(self):
        ...
        resp = self.app.get(
            "/login_handler?login=manager&password=badpassword", status=302
        )
        resp = resp.follow(status=200)
